...
describe('setup storage', () => {
  ...
  it('drops unknown categories and falls back to defaults when none remain', () => {
    const some = sanitizeSetup({ version: 1, players: [], categoryIds: ['food', 'nope', 'food'], impostorCount: 1 });
    expect(some.categoryIds).toEqual(['food']);
    const none = sanitizeSetup({ version: 1, players: [], categoryIds: ['nope'], impostorCount: 1 });
    expect(none.categoryIds).toEqual(DEFAULT_SETUP.categoryIds);
    const missing = sanitizeSetup({ version: 1, players: [], impostorCount: 1 });
  ...
  it('clamps the impostor count to the restored roster', () => {
    ...
    expect(sanitizeSetup({ version: 1, players, categoryIds: ['food'], impostorCount: 3 }).impostorCount).toBe(1);
    expect(sanitizeSetup({ version: 1, players, categoryIds: ['food'], impostorCount: 'two' }).impostorCount).toBe(1);
    const seven = Array.from({ length: 7 }, (_, i) => ({ id: `p${i}`, name: `שחקן ${i}` }));
    expect(sanitizeSetup({ version: 1, players: seven, categoryIds: ['food'], impostorCount: 2 }).impostorCount).toBe(2);
  });
  ...
  it('round-trips a valid setup', () => {
    ...
    const state = {
      ...
      categoryIds: ['music', 'nature'] as const,
      impostorCount: 1,
    };
